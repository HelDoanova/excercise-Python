...
@pytest.mark.parametrize('rank_one, suit_one, rank_two, suit_two', [
    (3, "s", 8, "k"),
    (3, "t", 3, "p")
])
def test_less_then_or_equal_to(rank_one, suit_one, rank_two, suit_two):
    """
    Test porovnání, první karta je menší nebo rovna druhé kartě
    """
    t_card1 = card.Card(rank_one, suit_one)
    t_card2 = card.Card(rank_two, suit_two)
...
def test_greater_then_or_equal_to(rank_one, suit_one, rank_two, suit_two):
    """
    Test porovnání, první karta je větší nebo rovna druhé kartě
    """
    t_card1 = card.Card(rank_one, suit_one)
    t_card2 = card.Card(rank_two, suit_two)
    assert t_card1 >= t_card2
...
def test_bad_rank_change():
    """
    Pokud je zadána neplatná hodnota při změně ranku,
    musí třída vyhodit výjimku TypeError.
    """
    t_card = card.Card(3, "t")
    with pytest.raises(TypeError):
...
def test_bad_suit_change():
    """
    Pokud je zadána neplatná barva při změně suitu,
    musí třída vyhodit výjimku TypeError.
    """
    t_card = card.Card(3, "t")
    with pytest.raises(TypeError):
